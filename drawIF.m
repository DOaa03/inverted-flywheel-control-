function drawIF(y, l, r, figTitle, u, dt)
...
ax3 = subplot(2,2,3);
h_theta_p = plot(ax3, t(1), theta_p(1), 'LineWidth', 1.5, 'DisplayName', 'Angle $\theta_p$', 'Color', '#0072BD');
hold on;
refernce=plot(ax3,[t(1) t(end)], [0 0], 'k--','LineWidth',1, 'DisplayName','refernce$\theta_p$'); 
grid(ax3,'on');
xlabel(ax3,'Time (s)');
ylabel(ax3,'$\theta_p$ (rad/s)', 'Interpreter', 'latex');
title(ax3,'Pendulum Angle');
legend(ax3, 'show', 'Interpreter', 'latex', 'Location', 'southeast');
xlim(ax3,[t(1) t(end)]);
p_min = min(theta_p); p_max = max(theta_p);
ylim(ax3, [p_min - 0.1, p_max ]);


% subplot 4: Flyhweel  Angular Velocity
ax4 = subplot(2,2,4);
h_dot_theta_f = plot(ax4, t(1), dot_theta_f(1), 'LineWidth', 1.5,  'DisplayName', 'Ang Vel $\dot{\theta}_f$', 'Color', '#0072BD');
hold on;
refernce=plot(ax4,[t(1) t(end)], [0 0], 'k--','LineWidth',1, 'DisplayName','refernce$\dot{\theta}_f$'); 
grid(ax4,'on');
xlabel(ax4,'Time (s)');
ylabel(ax4,' $\dot{\theta}_f$ ( rad/s)', 'Interpreter', 'latex');
title(ax4,'Flywheel Angular Velocity');
legend(ax4, 'show', 'Interpreter', 'latex', 'Location', 'northeast');
xlim(ax4,[t(1) t(end)]);
