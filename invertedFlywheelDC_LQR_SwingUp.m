...
for n = t
    ...
    if abs(x(1)) < (15 * pi/180) 
        ...
        E_target =M1 ;
        K_gain = 100;     % Adjust this gain for performance
        ...
        dydt(1,1)=x(3);
        dydt(2,1)=x(4);
        dydt(3,1) = (M1/D)*sin(x(1)) + (-c1/D)*x(3) + ((c2+M4)/D)*x(4) - u*(M3/D);
        dydt(4,1) = (-M1/D)*sin(x(1)) + (c1/D)*x(3) - ((c2 + M3)*M2/D)*x(4) + u*(M3*M2/D);
